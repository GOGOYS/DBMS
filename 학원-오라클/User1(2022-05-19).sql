...
영어 과목을 영어 영문과으로 변경을 하고싶다.
SQL UPDATE 명령을 이용하면 간단히 해결할 수 있다.

하지만 현재 table에서 100개의 레코드(데이터)들이 변경되는 문제가 발생한다.
한번에 다량의 레코드의 데이터가 변경되는 경우
데이터 무결성, transaction등의 문제를 잉르킬 수 있다.
...
/*
tbl_score 테이블이 초기 오류로 인하여 과목명이 일반 문자열로 등록되었다.
만약 과목명을 변경 하려면 많은 데이터를 변경해야 하기 때문에 문제를 일으킬 것이다.

tbl_score 테이블의 설계를 변경하여
과목정보 테이블을 생성하고 과목정보와 연계하여 과목명을 볼 수 있도록 하겠다.
*/
...
--아래의 명령은 전체 데이터에 sc_sbcode 칼럼의 값을 001 문자열로 대체한다
--드디어 일주일간 야근이다
--경고: UPDATE와 DELETE 명령은 WHERE이 없을때 절대 실행 금지!!
UPDATE tbl_score
SET sc_sbcode = '001';
...
--tbl_subject의 과목명이 일치하지 않아
--UPDATE가 되지 않은 데이터가 있는지 확인
SELECT *
FROM tbl_score
WHERE sc_sbcode = '001';

--z칼럼을 처음추가한 상태에서는 null 값이 담겨 있으므로
...
--NOT NUll 제약조건: 값을 INSERT 할때 이 칼럼은 비워 놓아서는 안된다.
ALTER TABLE tbl_score MODIFY (sc_sbcode NOT NULL);
--이후부터 데이터를 INSERT할때 sc_sbcode 칼럼의 데이터가 없으면 INSERT가 거부된다.
...
--DBMS에서는 길이가 같은 문자열은 부들호 조건으로 결과를 조회할 수 있다.
SELECT *
FROM tbl_score
...
/*
    완전 JOIN
    main table과 참조(foregn) table 간에
    완전 참조 무결성이 유지되는 경우는 
    결과에 신뢰성이 유지된다.
    
    만약 main table에는 데이터가 있는데 참조하는 값이 참조 table에 없는 경우
    결과는 main table 외 데이터가 조회되지 않는다.
*/
...
--현제 tblsubject table에는 SB001 데이터가 삭제된 상태이다
...
--main table은 조건에 따라 데이터 전부를 나열하고
...
--다수의 테이블에 저장된 데이터의 개수를 한번에 확인하기

...
-- MYSQL은 DB와 관련된 명령을 실행할때
-- 가장먼저 어떤 DB를 사용할지를 알려줘야 한다

USE mydb;
DROP TABLE tbl_test;
-- TABLE을 생성할 때 칼럼의 이름은 사전에 있는
-- 키워드는 가급적 사용하지 않는것이 좋다.
-- 사전에 있는 키워드를 사용할때는 접두사를
-- 부착하여 사용하는 것이 좋다
...
/*
    ...
    MYSQL을 사용하는데 MYSQL에서 사용하는 방식이
    DBMS의 업준으로 정착이 된 상태이다.
...
-- 다중데이터 INSERT 명령
INSERT INTO tbl_test (t_name)
VALUES ('이몽룡'),('성춘향'),('장영자'),('서시');

SELECT * FROM tbl_test;
/*
tbl_test의 id 칼럼은 auto_increment로 설정되어 있기에
데이터를 insert 할때마다 자동으로 Sequence 값이 생성되어
추가된다.

5.x mysql에서는 AUTO_INCREMENT 칼럼에 이슈가 있었다
5.x 숫자 type은 주로 INT형을 사용하는데 정수 11자리이다
때문에 정수형 11자리 범위를 벗어나는 경우 데이터 INSERT가
불가능해지는 문제가 있었다.
5.7 버전 이후에는 BIGINT형 type이 도입되어 문제를 해결하였다.
DB table을 만들때 AUTO_INCREMENT 칼럼에는 반드시
BIGINT형 type을 지정하자.
...
INT : 4Byte : -217483648 ~ 2147483647
...
/*
    ...
    데이터 타입은 BIGINT로 설정
    가급적 PRIMARY 키로 설정할것
    PK아닌 경우는 반드시 NOT NULL과 UNIQUE를 설정한다
    PK 또는 NOT NULL이 설정되지 않았을 경우
    TABLE 생성에서 오류가 난다.
*/
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0101','길동아','컴공괴',3,'010-111-1111','광주광역시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0102','이바보야','진짜아니야',3,'010-222-1458','아직도나를');
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S1010','ds','we',2,'ewew','ew');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0109','이바바','컴공과',3,'010-2222-5555','아 그');
